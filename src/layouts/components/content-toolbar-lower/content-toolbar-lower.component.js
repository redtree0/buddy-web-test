		...
		return (
			<Toolbar classes={{ root: classes.toolbarClass }}>
				{menuItems.map((item) =>
					item.href ? (
						<Button
							className={({ isActive }) => `${classes.menuItem} ${isActive ? classes.activeMenuItem : ''}`}
							key={item.title}
							// exact
							component={NavLink}
							to={item.href}
						...
						item.children && (
							<div key={item.title}>
								...
								<Menu id={item.title} anchorEl={anchor} open={Boolean(open === item.title)} onClose={this.handleClose}>
									{item.children.map((child) => (
										<MenuItem
											// exact
											className={({ isActive }) => (isActive ? classes.activeMenuItem : '')}
											component={NavLink}
											to={child.href}
											key={child.title}
