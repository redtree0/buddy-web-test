...
const MenuNestedFunky = (props) => {
	const { classes, menu, onClose } = props
					...
					{menu &&
						...
						menu.children.map((item) => (
							<ListItem
								className={({ isActive }) =>
									`${classes['portal-category-list__item']} ${
										isActive ? classes['portal-category-list__item--active'] : ''
									}`
								}
								title={item.title}
								key={item.title}
							...
							>
								...
								<Typography variant="button" color="inherit" className={classes['portal-category-list__item__text']}>
									{item.title}
								</Typography>
							</ListItem>
						))}
