		...
		<Grid container spacing={0}>
			...
			<Grid item xs={10}>
				<Paper className="portal-pages__content-inner">
					...
					<p>
						Text fields allow users to input text and usually appear in forms. Users may enter text, numbers, or mixed-format
						types of input.
					</p>
					<h2>TextField</h2>
					<p>
					...
					<p>
						...
						<code>Input</code>, and <code>FormHelperText</code>) that you can leverage directly to significantly customize your
						form inputs.
					</p>
					<p>
						You might also have noticed that some native HTML input properties are missing from the
						<code>TextField</code> component. This is on purpose. The component takes care of the most used properties, then
						it's up to the user to use the underlying component shown in the following demo. Still, you can use
						<code>inputProps</code> (and <code>InputProps</code>, <code>InputLabelProps</code> properties) if you want to avoid
						some boilerplate.
					</p>
					<Demo index={2} js={ComponentsExample} raw={ComponentsExampleRaw} />
					<h2>Layout</h2>
					<p>
						<code>TextField</code>, <code>FormControl</code> allow the specification of margin to alter the vertical spacing of
						inputs. Using <code>none</code> (default) will not apply margins to the
						<code>FormControl</code>, whereas <code>dense</code> and <code>normal</code> will as well as alter other styles to
						meet the specification.
					</p>
					<Demo index={3} js={LayoutExample} raw={LayoutExampleRaw} />
					<h2>Input Adornments</h2>
					<p>
						<code>Input</code> allows the provision of <code>InputAdornment</code>. These can be used to add a prefix, a suffix
						or an action to an input.
					</p>
