	...
	render() {
		...
		return (
			<Card className={classes['portal-annual-performance-widget']}>
				<CardHeader
					action={
						<IconButton aria-owns={anchorEl ? 'store-menu' : null} aria-haspopup="true" onClick={this.handleClick}>
							<MoreVertIcon />
						</IconButton>
					}
				...
				<CardContent className={classes['portal-annual-performance-widget__chart']}>
					<Chart type="bar" data={this.state.barChartData} options={this.state.barChartOptions} legend={legendOptions} />
				</CardContent>
				<Menu id="store-menu" anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={this.handleClose}>
					<MenuItem key={1} onClick={this.onItemClick}>
						Day
					</MenuItem>
					<MenuItem key={2} onClick={this.onItemClick}>
						Annual
					</MenuItem>
					<MenuItem key={3} onClick={this.onItemClick}>
						Month
					</MenuItem>
					<MenuItem key={4} onClick={this.onItemClick}>
						Week
					</MenuItem>
				</Menu>
			</Card>
