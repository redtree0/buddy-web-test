...
const ChatList = props => {
	...
	const createDesktopListItem = chat => {
		...
		return (
			<ListItem title={chat.from.name} key={chat.id} onClick={onSelect(chat)} divider button>
				<Avatar alt={chat.from.name} src={`${process.env.PUBLIC_URL}/${chat.from.image}`} />
				<ListItemText primary={chat.from.name} secondary={moment(chat.messages[0].date).fromNow()} />
	...
	const createMobileListItem = chat => (
		<ListItem key={chat.id} onClick={onSelect(chat)} divider button>
			<Avatar alt={chat.from.name} src={`${process.env.PUBLIC_URL}/${chat.from.image}`} />
		</ListItem>
	)
