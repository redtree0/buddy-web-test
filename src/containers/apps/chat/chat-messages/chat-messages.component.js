			...
			<div className={scss['chat-wrapper']}>
				<div className={scss['messages-wrapper']}>
					{chat.messages.map((message, index) => {
						const messageClass = chat.from.email === message.from.email ? scss['message--regular'] : scss['message--flipped']

						return (
							<div key={index} className={classNames(scss.message, messageClass)}>
								<div className={scss['avatar-wrapper']}>
									<Avatar alt={message.from.name} src={`${process.env.PUBLIC_URL}/${message.from.image}`} />
				...
				<Card className={scss['send-card']}>
					<CardContent className={scss['send-card-content']}>
						<TextField fullWidth label="Type a message here" onChange={this.updateMessage} value={this.state.message} />
