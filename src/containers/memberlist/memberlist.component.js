			...
			.then((res) => {
				res.data.list &&
					res.data.list.forEach((el) => {
						el.amountTotal = el.amountTotal * 1
					})
				this.setState({ members: res.data.list, listTotal: res.data.total })
			})
	...
	downloadExcel = async () => {
		await axios
			...
			.then((res) => {
				...
				link.href = url
				link.setAttribute('download', '회원목록_' + moment(new Date()).format('YYYYMMDD') + '.xlsx')
		...
		return (
			<div className={scss.button_div}>
				<Button
					...
					onClick={() => this.navigate('/usage', { state: { member: row } })}
				>
					상세
				</Button>
			...
			return (
				<div onClick={() => this.dataSort(value)} style={{ cursor: 'pointer' }}>
					{value === 'seq'
						...
						: value === 'phone'
						? '전화번호'
						: value === 'deskUsageCount'
						? '좌석구매'
						: value === 'roomUsageCount'
						? '룸 예약'
						: value === 'lockerUsageCount'
						? '락커구매'
						: value === 'regMethod'
						? '경로'
						: value === 'wdate'
				...
				<div style={{ width: '100%' }}>
					...
					<div style={{ margin: md.mobile() ? '2% 5%' : null }}>
						...
						>
							...
							>
								좌석구매
								{/* { this.orderCheck('deskUsageCount') } */}
							</TableHeaderColumn>
							<TableHeaderColumn
								dataField="roomUsageCount"
								width="70px"
								// dataSort
								searchable={false}
								dataAlign="center"
							...
							>
								룸 예약
								{/* { this.orderCheck('roomUsageCount') } */}
							</TableHeaderColumn>
							<TableHeaderColumn
								dataField="lockerUsageCount"
								width="70px"
								// dataSort
								searchable={false}
								dataAlign="center"
							...
							>
								락커구매
								{/* { this.orderCheck('lockerUsageCount') } */}
							</TableHeaderColumn>
							<TableHeaderColumn
								...
								searchable={false}
								dataAlign="center"
								dataFormat={(cell) => (cell > 0 ? cell.toLocaleString() + '원' : '-')}
							>
								이용금액
							</TableHeaderColumn>
							<TableHeaderColumn
...
export default compose(withWidth(), withStyles(themeStyles, { withTheme: true }))(MemberRoute(MemberList))
