...
class extensionDialog extends React.Component {
	...
	handleSend = () => {
		let data = {
			...
			extensionHour: this.state.extensionHour,
			extensionMinute: this.state.extensionMinute
		}
				...
				<DialogContent>
					<div className={'row dialog_row'} style={{ marginTop: '20px', marginBottom: '-20px' }}>
						...
						<input
							...
							onKeyUp={this.removeChar}
							value={this.state.information}
							onChange={event => this.handleChange('extensionHour', event)}
						/>
						<input
							placeholder="분"
							className={classNames('form-control', 'col-md-4', 'extension_textarea')}
							...
							onKeyUp={this.removeChar}
							value={this.state.information}
							onChange={event => this.handleChange('extensionMinute', event)}
						/>
					</div>
				</DialogContent>
				<DialogActions>
